...
export function excerptIsOnPage(pages: string[], page: number | undefined, excerpt: string) {
  ...
  // Sütunlu sayfalarda `pdftotext` kelime aralarını farklı bölebiliyor; boşluksuz da denenir.
  return fragments.every((fragment) => window.includes(fragment) || compactWindow.includes(fragment.replace(/\s/g, "")));
}
 ...
 */
export function downgradeUnlocatedClaims<Output extends { claims: Claim[] }>(output: Output, pages: string[]) {
  let downgraded = 0;
  const claims = output.claims.map((claim) => {
    if (claim.confidence !== "verified") return claim;
    const paperRefs = claim.sourceRefs.filter((reference) => reference.sourceId === "paper");
